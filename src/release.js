...
export async function runReleaseCheck({ root = process.cwd() } = {}) {
  ...
  const checks = [
    ...
    check("package_files", ["README.md", "action.yml", "bin/", "schema/", "spec/", "src/", "docs/", "examples/"].every((item) => pkg.files?.includes(item)), "package files must include runtime docs/examples/spec"),
