...
export async function runReleaseCheck({ root = process.cwd() } = {}) {
  ...
  const checks = [
    ...
    check("package_bin", pkg.bin?.["agent-contract"] === "./bin/agent-contract.js" && await exists(root, "bin/agent-contract.js"), "package bin must point to CLI"),
    check("package_files", ["README.md", "action.yml", "bin/", "schema/", "src/", "docs/", "examples/"].every((item) => pkg.files?.includes(item)), "package files must include runtime docs/examples"),
    ...
    check("schema_docs", await exists(root, "docs/agent-contract-schema.md") && schema.$id === "https://agentcontract.dev/schema/v1", "schema docs and schema id must exist"),
